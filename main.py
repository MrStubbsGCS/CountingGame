...
def countWins(node, count, turn):
    ...
    if node.value>9:
        ...
        count = countWins(node.right, count, turn)
        count = countWins(node.left, count, turn)
        return count
test = Node(0, 0)
run = True
computer_num = 0
...
while run:
    ...
    if usernum>9:
        ...
        if usernum-computer_num == 2:
            ...
            test = test.right
        count = countWins(test.right, 0, 0)
        countL = countWins(test.left, 0, 0)
        if count>countL:
